...
export class jsLegacy_Class {
    ...
    async replace_Async(dirPath: string) {
        dirPath = path.resolve(dirPath);
        let fsPaths: Array<string> = await abFSMatcher.getPaths_Async([
            `${dirPath}/**/*.js`,
        ]);
    ...
    replaceImportsAndExports(fsPath:string, data: string) {
    ...
    replaceLeadingEmptyLines(data: string) {
        let data_Arr = data.split("\r\n");
