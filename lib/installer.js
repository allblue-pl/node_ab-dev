...
class installer_Class {
    ...
    async install_Async(pkgPath, installType, depPkgName = null) {
        ...
        if (depPkgName === null) {
            ...
            for (let i = 0; i < depPkgNames_New.length; i++) {
                ...
                if (installType === 'git') {
                    if (!(await this.#installAsync_Git(pkgPath, abInfo, depPkgName, depPkgPath)))
                        return;
                }
                else if (installType === 'link') {
                    if (!(await this.#installAsync_Link_NoCopy(pkgPath, abInfo, depPkgName, depPkgPath))) {
    ...
    //     }
    // }
    #getPkgNodeModulesFiles(pkgPath) {
    ...
    async #installAsync_Git(pkgPath, abInfo, depPkgName, depPkgPath) {
        ...
        if (fs.existsSync(depPkgPath)) {
            ...
            if (await helper.git_HasUnstagedChanges_Async(depPkgPath)) {
                console.log(`Cannot pull repo '${depPkgName}':`, abLog.cWarn('Unstaged changes.'));
                return false;
            }
        ...
        if (depPkgExist) {
            ...
            try {
                await repo.clone(abInfo.info.abDependencies[depPkgName].url, depPkgPath, ['-b', depPkgInfo.branch]);
    ...
    //     return true;
    // }
    #installAsync_Link(pkgPath, abInfo, depPkgName, depPkgPath) {
        return new Promise((resolve, reject) => {
            if (fs.existsSync(depPkgPath))
                abFS.rmdirRecursiveSync(depPkgPath);
            let childExec = childProcess.exec(`npm link ${depPkgName}`, { cwd: pkgPath, }, (error, stdout, stderr) => {
    ...
    async #installAsync_Link_NoCopy(pkgPath, abInfo, depPkgName, depPkgPath) {
        return new Promise((resolve, reject) => {
            if (fs.existsSync(depPkgPath))
                abFS.rmdirRecursiveSync(depPkgPath);
            let childExec = childProcess.exec(`npm link ${depPkgName}`, { cwd: pkgPath, }, (error, stdout, stderr) => {
                ...
                this.#validateLinkedBranch_Async(depPkgName, depPkgPath, abInfo.info.abDependencies[depPkgName].branch)
                    .then((result) => {
                    resolve(result);
                })
                    .catch((err) => {
    ...
    async #installAsync_Links(pkgPath, depPkgsList) {
        return await new Promise((resolve, reject) => {
            ...
            let childExec = childProcess.exec(`npm link ${depPkgNames_Str}`, { cwd: pkgPath, }, (error, stdout, stderr) => {
    ...
    //     });
    // }
    async #installNPMDependenciesAsync(abInfo, pkgPath) {
        ...
        let tempPath = path.join(pkgPath, '.ab-dev-temp');
        fs.writeFileSync(path.join(tempPath, 'package.json'), JSON.stringify(abDevMainPackageJSON));
        await new Promise((resolve, reject) => {
            childProcess.exec(`npm install --production`, { cwd: tempPath, }, (error, stdout, stderr) => {
        ...
        let pkgNodeModulesFiles = this.#getPkgNodeModulesFiles(path.join(pkgPath, 'node_modules'));
        ...
        if (fs.existsSync(abDevMainNodeModulesDirPath)) {
            let abDevMainFiles = this.#getPkgNodeModulesFiles(abDevMainNodeModulesDirPath);
            for (let depPkgName of abDevMainFiles) {
                ...
                if (depPkgName in abInfo.info.abDependencies) {
                    ...
                    if (depPkgName.charAt(0) === '@') {
                        ...
                        let depPkgPath_Part = path.join(pkgPath, 'node_modules', depPkgName_Arr[0]);
    ...
    //     }
    // }
    async #validateLinkedBranch_Async(depPkgName, depPkgPath, branch) {
