...
import jsLegacy from "./js-legacy/index.js";
...
export class abDev_Class {
    ...
    async exec_Async(args) {
        ...
        /* Execute */
        if (args[0] === '-v') {
            let json = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json')).toString());
            ...
            await installer.install_Async(process.cwd(), args[1], args.length > 2 ? args[2] : null);
        }
    ...
    async validate_Async(pkgPath) {
        ...
        if (!abLockInfo.isValid(abInfo)) {
            ...
            let abLockInfo_New = new ABLockInfo(abLockInfo.type, abInfo.getInfoHash());
