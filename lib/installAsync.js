...
async function installAsync_Git(pkgPath, abInfo, depPkgName, depPkgPath)
{
    ...
    if (depPkgExist) {
        try {
            repo = await nodegit.Repository.open(depPkgPath);
            ...
            console.log(`Fetched repo: '${depPkgName}'.`);
        ...
        try {
            repo = await nodegit.Clone(abInfo.info.abDependencies[depPkgName], 
                    depPkgPath);
...
async function installNPMDependenciesAsync(abInfo, pkgPath)
{
    ...
    for (let pkgDepPkgName of pkgNodeModulesFiles) {
        ...
        if (fs.existsSync(pkgDepPkgPath))
